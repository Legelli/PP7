...
Si realizza un contatore a 4 bit utilizzando l'integrato SN74LS93N; si riporta in Figura~\ref{fig:counter} il circuito corrispondente. Il componente è alimentato con una tensione continua di \SI{5}{V} erogata da uno dei due generatori, e si collegano entrambi gli ingressi di \texttt{RESET}, mediante un interruttore, alternativamente a massa o a \SI{5}{V}, per azzerare manualmente il contatore\footnote{Questo collegamento è visibile nella figura del circuito completo (Figura~\ref{fig:complete_circuit}), non nella Figura~\ref{fig:counter}.}. 
...
Si misurano inoltre i livelli di tensione in uscita al contatore mediante il multimetro: si resetta il contatore a 0000 e si misurano le tensioni delle uscite; quindi, mediante un clock manuale, lo si porta a 1111 e si ripete la misura. Si riportano in Tabella~\ref{tab:counter} le misure di tensione per i livelli logici alto e basso per i quattro output.
%COMMENTARE!!
...
\]
e che si ha, per quanto visto prima, $V(Q_i) = \SI{3.23 \pm 0.06}{V} \equiv \overline{V}_\text{in}$, ottenuto dalla media sulle quattro uscite, si ottiene la relazione
\[
V_o = -\frac{R'}{8R} \cdot 15 \cdot \overline{V}_\text{in},
\] 
...
\begin{itemize} %Lo spazio verticale è stato tolto con \setlist{noitemsep} nell'header, rimuovere quella riga per avere una lista classica
	\item[--] $R_1 = \SI{1.0045 \pm 111}{\kilo\ohm}$;
	\item[--] $R_2 = \SI{468.7 \pm 111}{\ohm}$,
\end{itemize}
